...
class BookingRepository extends BaseRepository
{
     ...
     * @param $is_need_delay
     */
   public function sendPushNotification($users, $jobId, $data, $messageText, $isDelayed)
    {
        $logger = new Logger('push_logger');
        $logger->pushHandler(new StreamHandler(storage_path('logs/push/laravel-' . date('Y-m-d') . '.log'), Logger::DEBUG));
        $logger->pushHandler(new FirePHPHandler());
        $logger->addInfo('Push send for job ' . $jobId, [$users, $data, $messageText, $isDelayed]);
        
        $appId = env('APP_ENV') == 'prod' ? config('app.prodOnesignalAppID') : config('app.devOnesignalAppID');
        $apiKey = env('APP_ENV') == 'prod' ? config('app.prodOnesignalApiKey') : config('app.devOnesignalApiKey');
        $onesignalRestAuthKey = sprintf("Authorization: Basic %s", $apiKey);
    ...
    public function updateJob($id, $data, $cuser)
    {
        ...
        $current_translator = $job->translatorJobRel->first(function ($t) {
            return is_null($t->cancel_at);
        }) ?: $job->translatorJobRel->first(function ($t) {
        ...
        $change = $this->changeDetails($current_translator, $data, $job);
        if ($change['translatorChanged']) $log_data[] = $change['log_data'];
        ...
        $statusChange = $this->changeStatus($job, $data, $change['translatorChanged']);
        if ($statusChange['statusChanged']) $log_data[] = $statusChange['log_data'];
        ...
        if ($change['dateChanged']) $this->sendChangedDateNotification($job, $old_time);
        if ($change['translatorChanged']) $this->sendChangedTranslatorNotification($job, $current_translator, $change['new_translator']);
...
//        }
    ...
    public function sendSessionStartReminder($user, $job, $language, $due, $duration)
    {
        ...
        $due_exploded = explode(' ', $due);
        $location = $job->customer_physical_type === 'yes' ? 'på plats i ' . $job->town : 'telefon';
        $msg_text = [
            "en" => "Detta är en påminnelse om att du har en $language tolkning ($location) kl " . 
                     $due_exploded[1] . " på " . $due_exploded[0] . 
                     " som vara i $duration min. Lycka till och kom ihåg att ge feedback efter utförd tolkning!"
        ];

        if ($this->bookingRepository->shouldSendPush($user->id)) {
            $this->bookingRepository->sendPushNotification([$user], $job->id, $data, $msg_text, 
                                                            $this->bookingRepository->shouldDelayPush($user->id));
            $this->logger->addInfo('sendSessionStartReminder ', ['job' => $job->id]);
        }
    }
...
public function sendNotificationByAdminCancelJob($job_id)
{
    ...
    $data = [
        ...
        'status' => $job->status,
        'gender' => $job->gender == 'male' ? 'Man' : 'Kvinna',
        'certified' => $job->certified == 'both' ? ['normal', 'certified'] : [$job->certified == 'yes' ? 'certified' : $job->certified],
        'due' => $job->due,
        'job_type' => $job->job_type,
     ...
     * @param $user
     */
public function acceptJob($data, User $user)
    {
        $jobId = $data['job_id'];
        ...
        if (!Job::isTranslatorAlreadyBooked($jobId, $user->id, $job->due)) {
            if ($job->status === 'pending' && Job::insertTranslatorJobRel($user->id, $jobId)) {
                ...
                $email = $job->user_email ?? $customer->email;
                $name = $customer->name;
                $subject = "Bekräftelse - tolk har accepterat er bokning (bokning #$job->id)";
...
public function cancelJobAjax($data, $user)
    {
        $cancellationTime = Carbon::now();
        $job = Job::findOrFail($data['job_id']);
        $translator = Job::getJobsAssignedTranslatorDetail($job);
        $response = [];

        if ($user->is('customer')) {
            $job->withdraw_at = $cancellationTime;
            $job->status = $cancellationTime->diffInHours($job->due) >= 24 ? 'withdrawbefore24' : 'withdrawafter24';
            $job->save();
            ...
            if ($job->due->diffInHours($cancellationTime) > 24) {
                $customer = $job->user()->first();
                if ($customer) {
                    $data = [
                        'notification_type' => 'job_cancelled',
...
{
    $job_ids = Job::getJobsByUser($cuser);
    $job_ids = $job_ids->filter(function ($job) use ($cuser) {
        return $this->filterJobByTranslatorType($cuser, $job)
            && $this->filterJobByLanguage($cuser, $job)
            && $this->filterJobByGender($cuser, $job)
            && $this->filterJobByTranslatorLevel($cuser, $job)
            && $this->filterJobBySpecificAssignment($cuser, $job)
            && $this->filterJobByCustomerPhoneType($job)
            && $this->filterJobByCustomerPhysicalLocation($cuser, $job);
    });
    return $job_ids;
}

private function filterJobByTranslatorType(User $cuser, Job $job)
{
    $translator_type = $cuser->userMeta->translator_type;
    switch ($translator_type) {
        ...
        case 'rwstranslator':
            return $job->type === 'rws';
        case 'volunteer':
            return $job->type === 'unpaid';
        default:
            return false;
    }
}

private function filterJobByLanguage(User $cuser, Job $job)
{
    $languages = UserLanguages::where('user_id', $cuser->id)->pluck('lang_id');
    return in_array($job->lang_id, $languages);
}

private function filterJobByGender(User $cuser, Job $job)
{
    return $job->gender === $cuser->userMeta->gender;
}

private function filterJobByTranslatorLevel(User $cuser, Job $job)
{
    return $job->translator_level === $cuser->userMeta->translator_level;
}

private function filterJobBySpecificAssignment(User $cuser, Job $job)
{
    if (Job::assignedToPaticularTranslator($cuser->id, $job->id) === 'SpecificJob') {
        return Job::checkParticularJob($cuser->id, $job) !== 'userCanNotAcceptJob';
    }
    return true;
}

private function filterJobByCustomerPhoneType(Job $job)
{
    return $job->customer_phone_type !== 'no' && $job->customer_phone_type !== '';
}

private function filterJobByCustomerPhysicalLocation(User $cuser, Job $job)
{
    return $job->customer_physical_type !== 'yes' || Job::checkTowns($job->user_id, $cuser->id);
}
...
public function ignore($id, $model)
{
    if ($model === 'Job') {
        $instance = Job::find($id);
        $instance->ignore = 1;
    } elseif ($model === 'Throttles') {
        $instance = Throttles::find($id);
        $instance->ignore = 1;
    } elseif ($model === 'JobExpired') {
        $instance = Job::find($id);
        $instance->ignore_expired = 1;
    } else {
